...
curl -s -X POST "http://localhost:8080/api/v1/auth/signup" -H "Content-Type: application/json" -d @test/fixtures/auth_signup_master.json > mestre_response.tmp
type mestre_response.tmp

REM Se usuário já existe, fazer login
curl -s -X POST "http://localhost:8080/api/v1/auth/login" -H "Content-Type: application/json" -d @test/fixtures/auth_signup_master.json > mestre_login.tmp
echo ✅ Mestre autenticado
echo.

REM Registrar jogador
echo 3. Registrando usuário jogador...
curl -s -X POST "http://localhost:8080/api/v1/auth/signup" -H "Content-Type: application/json" -d @test/fixtures/auth_signup_player.json > jogador_response.tmp
type jogador_response.tmp

REM Se usuário já existe, fazer login
curl -s -X POST "http://localhost:8080/api/v1/auth/login" -H "Content-Type: application/json" -d @test/fixtures/auth_signup_player.json > jogador_login.tmp
